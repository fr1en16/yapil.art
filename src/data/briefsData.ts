...
export interface BriefQuestion {
  name: string;
  label: string;
  type: 'text' | 'tel' | 'url' | 'number' | 'textarea' | 'radio' | 'checkbox' | 'select';
  required?: boolean;
  placeholder?: string;
  hint?: string;
  options?: BriefOption[];
    ...
    groups: [
      {
        ...
        questions: [
          { name: 'site_type', label: 'Какой сайт нужен?', type: 'radio', options: [
          ...
          { name: 'business_goal', label: 'Какую бизнес-задачу должен решить сайт?', type: 'textarea', placeholder: 'Например: получать заявки, объяснять сложный продукт, продавать онлайн…' },
          { name: 'primary_action', label: 'Главное действие посетителя', type: 'text', placeholder: 'Оставить заявку, купить, записаться, скачать…' },
      ...
      },
      {
        ...
        questions: [
          { name: 'audience', label: 'Кто ваша основная аудитория?', type: 'textarea', placeholder: 'Сегменты, должности, география, уровень знакомства с продуктом' },
          { name: 'offer', label: 'Что именно вы предлагаете?', type: 'textarea' },
    ...
    groups: [
      {
        title: 'Контекст бренда', intro: 'Разберём исходную точку и задачу изменений.', questions: [
          { name: 'brand_stage', label: 'На каком этапе бренд?', type: 'radio', options: [
          ...
          { name: 'brand_name', label: 'Название бренда', type: 'text' },
      ...
      },
      {
        title: 'Стратегия и характер', intro: 'Сформулируем смысл, который должна передавать визуальная система.', questions: [
          { name: 'product', label: 'Что продаёт или делает бренд?', type: 'textarea' },
          { name: 'audience', label: 'Кто принимает решение о покупке?', type: 'textarea' },
      ...
      },
      {
        title: 'Визуальный охват', intro: 'Определим состав системы и реальные точки контакта.', questions: [
          { name: 'deliverables', label: 'Что нужно разработать?', type: 'checkbox', options: [
          ...
          { name: 'touchpoints', label: 'Где бренд встречается с аудиторией чаще всего?', type: 'textarea', placeholder: 'Сайт, вывеска, упаковка, документы, соцсети…' },
  ...
  print: {
    ...
    groups: [
      { title: 'Тип проекта', intro: 'Определим носитель и его задачу.', questions: [
        { name: 'print_type', label: 'Что нужно разработать?', type: 'radio', options: [
        ...
        { name: 'purpose', label: 'Какую задачу должен решить материал?', type: 'textarea' },
        { name: 'audience_context', label: 'Кто, где и как будет им пользоваться?', type: 'textarea' },
      ...
      { title: 'Производство', intro: 'Проверим параметры, от которых зависит подготовка макета.', questions: [
        { name: 'dimensions', label: 'Формат и размеры', type: 'text', placeholder: 'A4, 210×297 мм, коробка 120×80×40 мм…' },
      ...
      { title: 'Контент и стиль', intro: 'Соберём материалы и визуальные ориентиры.', questions: [
        { name: 'content_status', label: 'Тексты и обязательная информация готовы?', type: 'radio', options: yesNo },
  ...
  presentations: {
    ...
    groups: [
      { title: 'Сценарий', intro: 'Контекст показа определяет структуру и плотность слайдов.', questions: [
        { name: 'presentation_type', label: 'Тип презентации', type: 'radio', options: [
        ...
        { name: 'audience', label: 'Кто аудитория?', type: 'textarea' },
        { name: 'desired_action', label: 'Что аудитория должна понять или сделать после просмотра?', type: 'textarea' },
      ...
      { title: 'Материалы', intro: 'Поймём, насколько глубоко нужно работать со смыслом.', questions: [
        { name: 'content_stage', label: 'В каком состоянии контент?', type: 'radio', options: [
  ...
  smm: {
    ...
    groups: [
      { title: 'Каналы и задача', intro: 'Определим площадки и роль дизайна в контенте.', questions: [
        { name: 'platforms', label: 'Для каких площадок нужен дизайн?', type: 'checkbox', options: [
        ...
        { name: 'goal', label: 'Главная цель контента', type: 'textarea', placeholder: 'Узнаваемость, продажи, экспертность, запуск продукта…' },
        { name: 'account_links', label: 'Ссылки на действующие аккаунты', type: 'textarea' },
        { name: 'audience', label: 'Кто ваша аудитория?', type: 'textarea' },
      ...
      { title: 'Контент-система', intro: 'Соберём рабочий набор форматов, а не случайные макеты.', questions: [
        { name: 'work_format', label: 'Какой формат работы нужен?', type: 'radio', options: [
        ...
        { name: 'rubrics', label: 'Основные рубрики и типы публикаций', type: 'textarea' },
      ...
      { title: 'Визуальная база', intro: 'Проверим, что уже можно использовать и что предстоит создать.', questions: [
        { name: 'identity_status', label: 'Есть брендбук или сложившийся стиль?', type: 'radio', options: yesNo },
  ...
  support: {
    ...
    groups: [
      { title: 'Поток задач', intro: 'Зафиксируем объём и типичную нагрузку.', questions: [
        { name: 'task_types', label: 'Какие задачи возникают регулярно?', type: 'checkbox', options: [
        ...
        { name: 'task_examples', label: 'Примеры задач за последний месяц', type: 'textarea' },
      ...
      { title: 'Команда и процесс', intro: 'Настроим удобный канал постановки и согласования.', questions: [
        { name: 'stakeholders', label: 'Кто ставит и согласовывает задачи?', type: 'textarea' },
      ...
      { title: 'Визуальная система', intro: 'Проверим готовность материалов и зоны ответственности.', questions: [
        ...
        { name: 'first_month', label: 'Какие задачи нужно закрыть в первый месяц?', type: 'textarea' },
      ] },
    ],
  },
};
